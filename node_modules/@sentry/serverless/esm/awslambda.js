...
import { logger, isString, extractTraceparentData, baggageHeaderToDynamicSamplingContext, dsnFromString, dsnToString } from '@sentry/utils';
...
function tryPatchHandler(taskRoot, handlerPath) {
  ...
  if (!match) {
    (typeof __SENTRY_DEBUG__ === 'undefined' || __SENTRY_DEBUG__) && logger.error(`Bad handler ${handlerDesc}`);
    return;
  }
  ...
  try {
    ...
    (typeof __SENTRY_DEBUG__ === 'undefined' || __SENTRY_DEBUG__) && logger.error(`Cannot require ${handlerPath} in ${taskRoot}`, e);
    return;
  }
  ...
  if (!obj) {
    (typeof __SENTRY_DEBUG__ === 'undefined' || __SENTRY_DEBUG__) && logger.error(`${handlerPath} is undefined or not exported`);
    return;
  }
  if (typeof obj !== 'function') {
    (typeof __SENTRY_DEBUG__ === 'undefined' || __SENTRY_DEBUG__) && logger.error(`${handlerPath} is not a function`);
    return;
  }
...
export { defaultIntegrations, init, tryPatchHandler, wrapHandler };
